module PubUsersSearchHelper
  def self.get_search_where_sql(search_text, show_hidden = false)
    ...
    # end
    # where_sql
    get_search_sql(search_text, default_fields, { init_sql: show_hidden ? '(1)' : '(users.deleted_at IS NULL)' })
  end
